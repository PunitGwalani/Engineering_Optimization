...
calendar_date_end = datetime.datetime(2024, 1, 1, 00, 00, 00)
...
final_delta_v = np.zeros((epoch_size, tof_size))

for i in range(epoch_size):
    for j in range(tof_size):
        final_delta_v[i, j] = compute_total_delta_v(X[0, i], Y[j, 0])/1000

# Saving the data generated
np.savetxt('./Results101/Final_DepartureEpochs.dat', X)
np.savetxt('./Results101/Final_TimeOfFlight.dat', Y)
np.savetxt('./Results101/Final_DeltaV.dat', final_delta_v)
